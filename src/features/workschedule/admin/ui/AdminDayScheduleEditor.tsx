...
type AdminDayScheduleEditorProps = {
  date: Date;
...
const WORK_OPTIONS: {
...
}[] = [
  {
    ...
    icon: "business",
    color: "#dc2626",
    selectedBox: "border-red-300 bg-red-50",
  },
  {
    value: "remote",
    label: "Làm từ xa",
          ...
          return (
            <Pressable
              className={`mb-3 w-[48.5%] rounded-2xl border p-3 ${
                selected ? option.selectedBox : "border-slate-200 bg-slate-50"
              } ${readOnly ? "opacity-60" : ""}`}
              disabled={readOnly}
              key={option.value}
      ...
      {hasWorkSelection ? (
        <View>
                  ...
                  >
                    <Text
                      className={`text-[11px] font-black ${
                        selected ? "text-red-700" : "text-slate-500"
                      }`}
                    >
                      {option.label}
                    </Text>
          ...
          <View className="mt-4">
            ...
            <TextInput
              className={`min-h-12 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 ${
                readOnly ? "opacity-60" : ""
              }`}
              editable={!readOnly}
              maxLength={200}
